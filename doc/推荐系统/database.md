...
|pic|text|图片|
|video|text|视频|
|digest|varchar(500)|摘要|
|author|varchar(50)|作者|
|article_time|varchar(50)|文章日期|
|spider_time|timestamp|抓取日期|
|like_count|int(11)|喜欢数量|
|dis_like_count|int(11)|不喜欢数量|
|comment_count|int(11)|评论数量|

|索引名称|索引类型|索引字段|索引注释|
|----|------|----|---|
|PRIMARY|普通索引|id|无|
|office_keyword_title|普通索引|office,key_word,title|无|
...
|----|------|----|
|id|int(11)|自增id(主键)|
|content|varchar(128)|标签内容|
|ratio|int(11)|标签热度|
|type|int(4)|0：普通标签 1：用户推荐标签|
|order|int(11)|频道推荐顺序，只有type==1的时候起作用|
...
|user_id|int(11)|用户id|
|tag_id|int(11)|标签id|
|tag_type|int(3)|标签类型:0:用户添加 1:系统推荐，2：用户默认推荐|
...
|----|------|----|---|
|PRIMARY|唯一索引|id|无|
|uni_user_tag_rss|普通索引|user_id,tag_id|无|
|idx_tag_user_rss|普通索引|tag_id|无|
|idx_tag_user_type_order_rss|普通索引|user_id,tag_type,tag_order|无|
...
|mac|varchar(255)||
|idfa|varchar(255)||
|phone_model|varchar(36)|手机型号（iPhone6/红米Note4/p20/mate10)|
...
|device_sn|varchar(255)|设备串编号|
|temp_user_flag|int(1)|临时用户标记 （1为临时用户，0为真实用户）|
